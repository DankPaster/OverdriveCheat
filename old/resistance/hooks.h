...
namespace Resolver
{
	...
	void ResolvePlayer(CBaseEntity* pLocal, CBaseEntity* pEntity)
	{
		...
		if (pCurResolverEnt->m_iShotsFired() > 0)
		{
			...
			case 3: pCurResolverEnt->m_angEyeAngles() = QAngle(pEntity->m_angEyeAngles().pitch, pEntity->m_flLowerBodyYawTarget() - 90.0f, 0); break;
			case 4: ResolverStages[pCurResolverEnt->Index()] = 0; break;
			}
...
void DoAntiAimY(QAngle& angle, bool bFlip, bool& clamp) // yaw
{
	...
	case ANTIAIM_YAW_FAKE_SIDE_LBY:
		bFlip ? angle.yaw += zeta(g_LocalPlayer->m_flLowerBodyYawTarget()) : zeta(g_LocalPlayer->m_flLowerBodyYawTarget() + 60.0f); // zeta of lby
		break;
	}
}
	...
	ImGui::Begin("OVERDRIVE.ME", &visible, ImVec2(630, 375), 1.00f, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_ShowBorders | ImGuiWindowFlags_AlwaysAutoResize);
	{
		...
		case TAB_AIMBOT:
			...
			case AIMBOTTAB_AIMBOT:
				...
				ImGui::Combo("HITBOX", &g_Options.aimbot_hitbox, hitboxes, ARRAYSIZE(hitboxes));
				ImGui::SliderFloat("AIMBOT FOV", &g_Options.aimbot_AimbotFOV, 0.0f, 180.0f);
				ImGui::SliderFloat("AIMBOT SMOOTH", &g_Options.aimbot_smooth, 0.0f, 1.0f);
...
void RenderFrame(IDirect3DDevice9* device)
{
	...
	if (g_EngineClient->IsInGame() && g_LocalPlayer)
	{
		...
		{
			float Radius;
			Radius = tanf((DEG2RAD(g_Options.aimbot_AimbotFOV)) / 6) / tanf(97) * w;
			g_Renderer->AddCircle(ImVec2(w / 2, h / 2), Radius, D3DCOLOR_RGBA(0, 255, 0, 255), g_Options.aimbot_AimbotFOV * 10.0f, 0.5f);
		}
		// Watermark
...
void DoAimbot(CUserCmd* pCmd)
{
	...
	if (pClosest && pClosest->m_iTeamNum() != g_LocalPlayer->m_iTeamNum() && !pClosest->m_bDormant() && pClosest->IsAlive() && !pClosest->IsImmune())
	{
		...
		Vector BonePos;
		switch (g_Options.aimbot_hitbox)
		{
...
void __stdcall hkCreateMove(int sequence_number, float input_sample_frametime, bool active, bool& bSendPacket)
{
	...
	// Aimbot
	if (g_Options.aimbot_AimbotFOV > 0.0f && g_EngineClient->IsInGame() && g_LocalPlayer->IsAlive() && g_LocalPlayer->m_iShotsFired() >= g_Options.aimbot_shots) DoAimbot(pCmd);
